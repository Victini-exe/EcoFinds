...
app.config.update(
    ...
    MAIL_PORT=587,
    MAIL_USE_TLS=True,
    MAIL_USERNAME='hariniradhu@gmail.com',  
    MAIL_PASSWORD='nzbk vozg jedj gqkm',     
...
def signup():
    ...
    # Generate OTP and send email right after user creation
    ...
    msg = Message("Your EcoFinds OTP",
                  ...
                  recipients=[email])
    msg.body = f"Welcome to EcoFinds! Your OTP is {otp}. It is valid for 5 minutes."

    try:
        mail.send(msg)
    except Exception as e:
        print(f"[ERROR] Mail sending failed: {e}")
        return jsonify({"error": "User created but failed to send OTP email"}), 500

    return jsonify({"message": f"User {email} created and OTP sent. Please verify your email."}), 201
